...
class RSAEngine extends BaseAsymmetricBlockCipher {
  ...
  BigInt _convertInput(Uint8List inp, int inpOff, int len) {
    ...
    var res = BigIntUtil.fromBytes(1, inp.sublist(inpOff, inpOff+len));
    if (res >= _key.modulus) {
      throw new ArgumentError("Input too large for RSA cipher");
    }
  ...
  int _convertOutput(BigInt result, Uint8List out, int outOff) {
    final output = BigIntUtil.toBytes(result);
  ...
  BigInt _processBigInteger(BigInt input) {
    if (_key is RSAPrivateKey) {
      ...
      mP = BigIntUtil.modPow(input.remainder(privKey.p), _dP, privKey.p);

      mQ = BigIntUtil.modPow(input.remainder(privKey.q), _dQ, privKey.q);
      ...
      return BigIntUtil.modPow(input, _key.exponent, _key.modulus);
    }
  }
